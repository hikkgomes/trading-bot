...
def train_signal_model(train: pd.DataFrame, features: Sequence[str], label_column: str):
    ...
    model = lgb.LGBMClassifier(
        n_estimators=1000, learning_rate=0.03, num_leaves=31, subsample=0.9,
        colsample_bytree=0.9, random_state=42, n_jobs=-1, verbosity=-1,
    )
    model.fit(
        x.iloc[:val_split], y.iloc[:val_split],
...
def predict_signals(model, data: pd.DataFrame, features: Sequence[str], tp: float, sl: float, fee_cost: float, min_ev: float = 0.0):
    x = data.loc[:, list(features)].replace([np.inf, -np.inf], np.nan).fillna(0.0)
    prob = model.predict_proba(x)[:, 1]
    ev = prob * (tp - fee_cost) - (1.0 - prob) * (sl + fee_cost)
    return pd.Series(ev > min_ev, index=data.index), pd.Series(prob, index=data.index), pd.Series(ev, index=data.index)
...
def walk_forward_model_signals(
    ...
    for train_slice, test_slice in windows:
        ...
        fold_features = screen_features(train, label_column, features, max_features=max_features, method="shap")
...
def backtest_model_signals(data: pd.DataFrame, signals_df: pd.DataFrame, trade_config, base_prefix: str = "tf_5m_") -> pd.DataFrame:
    ...
    direction_match = re.search(r"label_(long|short)_", str(signals_df.get("label_column", pd.Series([""])).iloc[0]))
...
def run(
    ...
    resolved_embargo = embargo_bars if embargo_bars is not None else _infer_embargo_bars(label_column, fallback=16)
    wf = WalkForwardConfig(
        train_bars=defaults["train_bars"], test_bars=defaults["test_bars"], step_bars=defaults["step_bars"], embargo_bars=resolved_embargo
    )
    ...
    if report:
        ...
        (output_dir / "report.json").write_text(pd.Series(summary).to_json(indent=2), encoding="utf-8")
    return output_path


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Walk-forward model-driven signals.")
    parser.add_argument("--input-path", type=Path, default=PROCESSED_DATA_DIR / "train_5m_indicators_labels.parquet")
    parser.add_argument("--output-dir", type=Path, default=PROJECT_ROOT / "outputs" / "model_signals")
...
def main() -> None:
    args = parse_args()
    out = run(
        args.input_path, args.output_dir, args.base_timeframe, args.label_column,
        args.tp, args.sl, args.fee_bps, args.slippage_bps, args.embargo_bars, args.report,
    )
    print(f"Wrote {out}")
