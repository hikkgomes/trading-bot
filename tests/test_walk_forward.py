...
def test_generate_windows_no_overlap():
    cfg = WalkForwardConfig(
        train_bars=20, test_bars=10, step_bars=10, min_windows=2, embargo_bars=3
    )
...
def test_generate_windows_embargo_zero():
    cfg = WalkForwardConfig(
        train_bars=20, test_bars=10, step_bars=10, min_windows=2, embargo_bars=0
    )
    windows = generate_windows(80, cfg)
...
def test_refit_conditions_recomputes_quantiles():
    ...
    candidate = StrategyCandidate(
        ...
        conditions=(
            Condition(
                "tf_5m_rsi_14", "value_ge", 0.0, "desc", threshold_source="quantile", quantile=0.9
            ),
        ),
    )
...
def test_compute_threshold_sources():
    ...
    assert (
        _compute_threshold(
            df, Condition("a", "value_ge", 0.0, "d", threshold_source="quantile", quantile=0.8)
        )
        > 0
    )
    assert (
        _compute_threshold(
            df,
            Condition("a", "delta_ge", 0.0, "d", threshold_source="delta_quantile", quantile=0.9),
        )
        >= 0
    )
    assert (
    ...
    assert (
        _compute_threshold(df, Condition("a", "cross_above", 0.0, "d", threshold_source="fixed"))
        == 0.0
    )
...
def test_compute_threshold_missing_metadata_raises():
    ...
    with pytest.raises(ValueError):
        _compute_threshold(
            df, Condition("a", "value_ge", 0.0, "d", threshold_source="quantile", quantile=None)
        )
    with pytest.raises(ValueError):
        _compute_threshold(
            df,
            Condition("a", "ratio_ge", 0.0, "d", threshold_source="ratio_quantile", quantile=0.8),
        )
    with pytest.raises(ValueError):
    ...
    with pytest.raises(ValueError):
        _compute_threshold(
            df,
            Condition("a", "slope_3_ge", 0.0, "d", threshold_source="slope_quantile", quantile=0.9),
        )
...
def test_walk_forward_score_candidate_calls_each_fold():
    ...
    candidate = StrategyCandidate(
        "long",
        4,
        (Condition("x", "value_ge", 0.0, "x", threshold_source="quantile", quantile=0.8),),
    )
    ...
    def score_fn(train, test, cand, conf, base_prefix):
        _ = conf, base_prefix
        return {
            "test_total_return": 1.0 if cand.conditions[0].threshold > 0 else 0.0,
            "test_avg_net_return": 0.01,
