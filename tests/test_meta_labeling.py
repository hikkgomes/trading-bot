...
def test_meta_labeling_passes_fee_cost(tmp_path, monkeypatch):
    ...
    def fake_walk_forward_model_signals(
        data, features, label_column, wf, tp, sl, fee_cost, max_features=80
    ):
    ...
    monkeypatch.setattr(
        meta_labeling, "walk_forward_model_signals", fake_walk_forward_model_signals
    )
