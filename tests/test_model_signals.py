...
def test_predict_signals_positive_ev_filter():
    ...
    mask, prob, ev = predict_signals(
        _Stub(), df, ["f"], tp=0.01, sl=0.01, fee_cost=0.001, min_ev=0.001
    )
    assert mask.sum() > 0
    assert (prob >= 0).all()
...
def test_walk_forward_model_signals_oos_rows_and_source_index():
    ...
    out = walk_forward_model_signals(
        df,
        ["f1", "f2"],
...
def test_backtest_model_signals_compatible_with_simulator():
    ...
    config = DayTradeConfig(
        take_profit=0.005, stop_loss=0.003, fee_bps=0, slippage_bps=0, horizon_bars=4
    )
