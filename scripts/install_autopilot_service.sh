...
READINESS_REPORT_UNIT="$(systemd_quote "$REPO/runtime/readiness_report.md")"
READINESS_JSON_UNIT="$(systemd_quote "$REPO/runtime/readiness_report.json")"
...
ExecStartPre=$PYTHON_UNIT -m src.autopilot.readiness --config $CONFIG_UNIT --output $READINESS_REPORT_UNIT --json-output $READINESS_JSON_UNIT
