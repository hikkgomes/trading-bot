...
# --------------------------------------------------------------------------- #
def trend_continuation(regime_tf: str, setup_tf: str, trigger_tf: str,
                       direction: str, idx: int) -> Hypothesis:
    long = direction == "long"
    regime = [
        Predicate(regime_tf, "ema_50", "gt_feature" if long else "lt_feature", feature_b="ema_200",
                  note=f"{regime_tf} EMA50 {'>' if long else '<'} EMA200 (trend {'up' if long else 'down'})"),
        Predicate(regime_tf, "adx_14", "ge", reference=20.0, note=f"{regime_tf} ADX>=20 (trend has force)"),
    ]
    setup = [
        Predicate(setup_tf, "close",
                  "lt_feature" if long else "gt_feature", feature_b="bbands_20_middleband",
                  note=f"{setup_tf} pulled back {'below' if long else 'above'} 20-band mean"),
        Predicate(setup_tf, "rsi_14", "le" if long else "ge", reference=48.0 if long else 52.0,
                  note=f"{setup_tf} RSI cooled ({'<=48' if long else '>=52'})"),
    ]
    trigger = [
        Predicate(trigger_tf, "macd_macd", "cross_above" if long else "cross_below",
                  feature_b="macd_macdsignal", note=f"{trigger_tf} MACD {'bull' if long else 'bear'} cross"),
        Predicate(trigger_tf, "volume_z_20", "ge", reference=0.5,
    ...
    return Hypothesis(
        ...
        idea=(f"In a {regime_tf} {'up' if long else 'down'}trend, {'buy' if long else 'sell'} {setup_tf} "
              f"pullbacks once {trigger_tf} momentum turns back {'up' if long else 'down'} on rising volume."),
        market_logic=("Trend-followers and dip buyers re-enter after controlled pullbacks; the higher "
                      "timeframe trend stacks the odds, the trigger times the resumption."),
        direction=direction,
        base_timeframe=trigger_tf, regime_timeframe=regime_tf,
        setup_timeframe=setup_tf, trigger_timeframe=trigger_tf,
        regime=regime, setup=setup, trigger=trigger,
        exit=ExitRule(take_profit=0.012, stop_loss=0.008, horizon_bars=_horizon(trigger_tf),
                      atr_take_profit=1.8, atr_stop_loss=1.2),
        risk=RiskRule(risk_per_trade=0.01, max_trades_per_day=4, cooldown_bars=3, min_atr_pct=0.0010),
        expected_holding="minutes to a few hours",
        expected_frequency="a few setups per day in trending regimes",
        invalidation=(f"{regime_tf} trend flips ({'EMA50<EMA200' if long else 'EMA50>EMA200'}) or the "
...
# --------------------------------------------------------------------------- #
def volatility_breakout(regime_tf: str, setup_tf: str, trigger_tf: str,
                        direction: str, idx: int) -> Hypothesis:
    long = direction == "long"
    regime = [
        Predicate(regime_tf, "natr_14", "q_le", quantile=0.35, window=180,
                  note=f"{regime_tf} volatility compressed (NATR in bottom 35% of last 180 bars)"),
    ]
    setup = [
        Predicate(setup_tf, "stddev_20", "q_le", quantile=0.40, window=120,
                  note=f"{setup_tf} range tightening (stddev in bottom 40%)"),
        Predicate(setup_tf, "adx_14", "le", reference=20.0,
                  note=f"{setup_tf} not yet trending (ADX<=20) — coiled"),
    ]
    trigger = [
        Predicate(trigger_tf, "close", "gt_feature" if long else "lt_feature",
                  feature_b="max_20" if long else "min_20", shift_b=1,
                  note=f"{trigger_tf} closes {'above the PRIOR 20-bar high' if long else 'below the PRIOR 20-bar low'} (range break)"),
        Predicate(trigger_tf, "volume_z_20", "ge", reference=1.0,
    ...
    return Hypothesis(
        ...
        idea=(f"After {regime_tf} volatility compresses and {setup_tf} range tightens, trade the first "
              f"{trigger_tf} {'upside' if long else 'downside'} break on a volume expansion."),
        market_logic=("Volatility mean-reverts: long quiet periods build up stop clusters and pent-up "
                      "order flow that fuel a directional expansion when the range finally breaks."),
        direction=direction,
        base_timeframe=trigger_tf, regime_timeframe=regime_tf,
        setup_timeframe=setup_tf, trigger_timeframe=trigger_tf,
        regime=regime, setup=setup, trigger=trigger,
        exit=ExitRule(take_profit=0.018, stop_loss=0.007, horizon_bars=_horizon(trigger_tf) + 12,
                      atr_take_profit=2.5, atr_stop_loss=1.0),
...
# --------------------------------------------------------------------------- #
def mean_reversion(regime_tf: str, setup_tf: str, trigger_tf: str,
                   direction: str, idx: int) -> Hypothesis:
    ...
    setup = [
        Predicate(setup_tf, "close", "lt_feature" if long else "gt_feature",
        ...
        Predicate(setup_tf, "rsi_14", "le" if long else "ge", reference=25.0 if long else 75.0,
                  note=f"{setup_tf} RSI {'<=25 oversold' if long else '>=75 overbought'}"),
    ]
    trigger = [
        Predicate(trigger_tf, "rsi_14", "rising" if long else "falling", lookback=3,
                  note=f"{trigger_tf} RSI turning back {'up' if long else 'down'} (reclaim begins)"),
        Predicate(trigger_tf, "close", "gt_feature" if long else "lt_feature", feature_b="open",
    ...
    return Hypothesis(
        ...
        idea=(f"When {regime_tf} trend is intact but {setup_tf} stretches to a band extreme and "
              f"{trigger_tf} starts reclaiming, scalp the snap-back."),
        market_logic=("Sharp counter-trend flushes overshoot fair value; once forced selling/buying "
                      "exhausts, price reverts to the mean. Only taken *with* the higher-TF trend."),
        direction=direction,
        base_timeframe=trigger_tf, regime_timeframe=regime_tf,
        setup_timeframe=setup_tf, trigger_timeframe=trigger_tf,
        regime=regime, setup=setup, trigger=trigger,
        exit=ExitRule(take_profit=0.008, stop_loss=0.006, horizon_bars=max(8, _horizon(trigger_tf) // 2),
                      atr_take_profit=1.2, atr_stop_loss=1.0),
        risk=RiskRule(risk_per_trade=0.0075, max_trades_per_day=5, cooldown_bars=4, max_atr_pct=0.02),
...
# --------------------------------------------------------------------------- #
def momentum_continuation(regime_tf: str, setup_tf: str, trigger_tf: str,
                          direction: str, idx: int) -> Hypothesis:
    long = direction == "long"
    regime = [
        Predicate(regime_tf, "close", "gt_feature" if long else "lt_feature", feature_b="ema_50",
                  note=f"{regime_tf} price {'above' if long else 'below'} EMA50 (directional pressure)"),
        Predicate(regime_tf, "adx_14", "ge", reference=25.0, note=f"{regime_tf} ADX>=25 (strong trend)"),
    ]
    setup = [
        Predicate(setup_tf, "rsi_14", "between", low=45.0 if long else 30.0, high=70.0 if long else 55.0,
                  note=f"{setup_tf} shallow pullback (RSI in continuation zone, not exhausted)"),
        Predicate(setup_tf, "close", "gt_feature" if long else "lt_feature", feature_b="ema_20",
                  note=f"{setup_tf} still {'above' if long else 'below'} EMA20"),
    ]
    trigger = [
        Predicate(trigger_tf, "mom_10", "slope_up" if long else "slope_down", lookback=3, reference=0.0,
                  note=f"{trigger_tf} momentum re-accelerating"),
        Predicate(trigger_tf, "volume_z_20", "ge", reference=0.5, note=f"{trigger_tf} participation picking up"),
    ]
    return Hypothesis(
        id=f"MOMCONT_{direction.upper()}_{regime_tf}_{setup_tf}_{trigger_tf}_{idx:03d}",
        family="momentum_continuation",
        idea=(f"Strong {regime_tf} momentum + a shallow {setup_tf} pause + a {trigger_tf} re-acceleration "
              f"= continuation entry in the trend direction."),
        market_logic=("Strong trends pause shallowly before continuing as latecomers and trend-followers "
                      "keep adding; momentum that refuses to give back much rarely reverses immediately."),
        direction=direction,
        base_timeframe=trigger_tf, regime_timeframe=regime_tf,
        setup_timeframe=setup_tf, trigger_timeframe=trigger_tf,
        regime=regime, setup=setup, trigger=trigger,
        exit=ExitRule(take_profit=0.010, stop_loss=0.006, horizon_bars=_horizon(trigger_tf),
                      atr_take_profit=1.6, atr_stop_loss=1.0),
...
# --------------------------------------------------------------------------- #
def liquidity_sweep(regime_tf: str, setup_tf: str, trigger_tf: str,
                    direction: str, idx: int) -> Hypothesis:
    ...
    setup = [
        Predicate(setup_tf, "low" if long else "high", "lt_feature" if long else "gt_feature",
                  feature_b="min_50" if long else "max_50", shift_b=1,
                  note=f"{setup_tf} {'pierces the PRIOR 50-bar low' if long else 'pierces the PRIOR 50-bar high'} (stop run)"),
    ]
    trigger = [
        Predicate(trigger_tf, "close", "gt_feature" if long else "lt_feature",
                  feature_b="min_20" if long else "max_20", shift_b=1,
                  note=f"{trigger_tf} reclaims back {'above the PRIOR 20-bar low' if long else 'below the PRIOR 20-bar high'}"),
        Predicate(trigger_tf, "rsi_14", "rising" if long else "falling", lookback=2,
                  note=f"{trigger_tf} momentum flips after the sweep"),
        Predicate(trigger_tf, "volume_z_20", "ge", reference=0.8, note=f"{trigger_tf} reclaim on real volume"),
    ]
    return Hypothesis(
        id=f"SWEEP_{direction.upper()}_{regime_tf}_{setup_tf}_{trigger_tf}_{idx:03d}",
        family="liquidity_sweep",
        idea=(f"A {setup_tf} stop-run through prior {'lows' if long else 'highs'} that immediately reclaims "
              f"on {trigger_tf}, inside a {regime_tf} {'up' if long else 'down'}trend."),
        market_logic=("Resting stops below obvious lows (above highs) are liquidity. Price spikes to grab "
                      "them, then snaps back; the failed breakdown traps breakout sellers and fuels reversal."),
        direction=direction,
        base_timeframe=trigger_tf, regime_timeframe=regime_tf,
        setup_timeframe=setup_tf, trigger_timeframe=trigger_tf,
        regime=regime, setup=setup, trigger=trigger,
        exit=ExitRule(take_profit=0.012, stop_loss=0.006, horizon_bars=_horizon(trigger_tf),
                      atr_take_profit=2.0, atr_stop_loss=0.9),
...
def no_trade_guard_predicates(
    tf: str, *, chop: bool = True, vol_floor: bool = True, vol_ceiling: bool = True,
    adx_min: float = 18.0, vol_window: int = 240,
) -> list[Predicate]:
    ...
    if chop:
        preds.append(Predicate(tf, "adx_14", "ge", reference=adx_min,
                               note=f"{tf} ADX>={adx_min:g} — enough directionality to bother (anti-chop)"))
    if vol_floor:
        preds.append(Predicate(tf, "natr_14", "q_ge", quantile=0.10, window=vol_window,
                               note=f"{tf} NATR above its rolling bottom-10% — not dead (unpayable after fees)"))
    if vol_ceiling:
        preds.append(Predicate(tf, "natr_14", "q_le", quantile=0.95, window=vol_window,
...
def apply_no_trade_guards(hyp: Hypothesis, guard_tf: str | None = None, **kw) -> Hypothesis:
    ...
    return Hypothesis(
        ...
        invalidation=hyp.invalidation + " Guards also stand the strategy aside in chop / dead / chaotic vol.",
...
FAMILIES: dict[str, Family] = {
    "trend_continuation": Family(
        "trend_continuation", "A. Trend continuation (pullback re-entry)",
        "HTF trend + MTF pullback + LTF momentum re-entry.",
        trend_continuation,
        [("1d", "4h", "30m"), ("4h", "1h", "15m"), ("4h", "30m", "5m"), ("1h", "30m", "5m")],
    ),
    "volatility_breakout": Family(
        "volatility_breakout", "B. Volatility breakout (compression -> expansion)",
    ...
    "mean_reversion": Family(
        "mean_reversion", "C. Mean reversion (stretched + reclaim)",
    ...
    "momentum_continuation": Family(
        "momentum_continuation", "D. Momentum continuation (shallow pullback)",
    ...
    "liquidity_sweep": Family(
        "liquidity_sweep", "E. Liquidity sweep / failed breakdown",
